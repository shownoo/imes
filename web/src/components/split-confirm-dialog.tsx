    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md gap-0 p-0 overflow-hidden">
        <DialogHeader className="border-b px-6 py-4 text-left">
          <DialogTitle>{t('拆零确认')}</DialogTitle>
          <DialogDescription>{t('核对在库数量后输入本次出库数量，拆零将自动生成剩余物资新码')}</DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 px-6 py-4">
          <div className="rounded-lg border bg-muted/30 p-4">
            <div className="flex items-start gap-3">
              ...
              <div className="text-right">
                <p className="text-xs text-muted-foreground">{t('在库')}</p>
          ...
          <div className="text-center">
            ...
            {willSplit && (
              <p className="mt-2 text-sm text-amber-700 dark:text-amber-400">{t('拆零后剩余')}<span className="font-semibold tabular-nums">{remaining}</span> {stock.unit}，将打印新标签
              </p>
            )}
            {invalid && pickedQty > 0 && (
