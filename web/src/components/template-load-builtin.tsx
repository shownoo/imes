...
export function TemplateLoadBuiltin({ templateKey }: { templateKey: string }) {
  ...
  return (
    <>
      <Button type="default" size="small" title='载入默认' onClick={() => setOpen(true)}>
        载入默认</Button>

      <AlertDialog open={open} onOpenChange={setOpen}>
        <AlertDialogContent className="max-w-md gap-3">
          <AlertDialogHeader>
            <AlertDialogTitle>载入系统默认模板？</AlertDialogTitle>
            <AlertDialogDescription className="text-left">当前编辑器中的内容将被系统内置模板替换，此操作不会自动保存。</AlertDialogDescription>
          </AlertDialogHeader>
