...
function HorizontalBarChart({
  ...
  if (!items.length) {
    return <p className="py-8 text-center text-sm" style={{ color: 'var(--leader-text-muted)' }}>暂无出库记录</p>
  }
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex gap-4 text-[10px]" style={{ color: 'var(--leader-text-muted)' }}>
        <span className="flex items-center gap-1"><span className="size-2 rounded-sm bg-primary/85" />入库</span>
        <span className="flex items-center gap-1"><span className="size-2 rounded-sm" style={{ background: 'hsl(var(--gold))' }} />出库</span>
      </div>
    </div>
  )
}
...
export function DashboardChartAlertPie({ charts }: { charts: DashboardChartsData }) {
  if (!charts.alertPie.length) {
    return <p className="py-8 text-center text-sm" style={{ color: 'var(--leader-text-muted)' }}>暂无未处理预警</p>
  }
...
export function DashboardChartDestinationBar({ charts }: { charts: DashboardChartsData }) {
  return (
    <>
      ...
      <p className="mt-3 text-[10px]" style={{ color: 'var(--leader-text-muted)' }}>
        按所属区汇总出库物资件数（已发运/已完成取实拣量，其余取申请量），并附单数；不含草稿/驳回
      </p>
    </>
  )
}
