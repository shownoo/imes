...
import { PARSE_INBOUND_TEXT } from '../inbound/queries'
...
interface UseOutboundDocumentImportOptions {
  onParsed: (result: OutboundParseResult) => void
}

interface UseOutboundDocumentImportReturn {
  isParsing: boolean
...
export function useOutboundDocumentImport(
  options: UseOutboundDocumentImportOptions,
): UseOutboundDocumentImportReturn {
  ...
  const runParse = useCallback(
    async (text: string, kind: DocumentImportKind) => {
      ...
      const raw = data?.parseInboundDocumentText as {
        rows?: Array<{ materialId: string; expectedQty: number; hint?: string; matchedBy?: string }>
        unmatched?: Array<{ hint: string; expectedQty: number }>
      } | undefined
      const rows = (raw?.rows ?? []).map((r) => ({
        materialId: r.materialId,
        requestedQty: r.expectedQty,
        hint: r.hint,
